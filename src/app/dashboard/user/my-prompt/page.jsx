...
const MyPromptPage = async () => {
    ...
    const userPromptsData = await getUserAddPrompts(user?.email) || [];
                                    ...
                                    <tr key={prompt._id?.$oid || prompt._id} className="hover:bg-slate-800/30 transition-all duration-200">
                                        ...
                                        <td className="px-6 py-4 max-w-sm">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="truncate">
                                                    <p className="font-semibold text-slate-100 truncate group-hover:text-white">{prompt.title}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <span className="text-xs text-blue-400 border-l border-slate-700 pl-2 font-medium tracking-wide">
                                                    {prompt.aiTool}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border backdrop-blur-sm ${prompt.status === 'pending'
                                                ...
                                                <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${prompt.status === 'pending' ? 'bg-amber-400' : 'bg-emerald-400'}`}></span>
